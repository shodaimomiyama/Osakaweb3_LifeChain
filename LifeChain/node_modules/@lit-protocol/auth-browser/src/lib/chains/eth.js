...
const connectWeb3 = async ({ chainId = 1, walletConnectProjectId, }) => {
    ...
    (0, misc_1.log)('got provider', provider);
    // @ts-ignore
 ...
 * @returns
 */
const checkAndSignEVMAuthMessage = async ({ chain, resources, switchChain, expiration, uri, walletConnectProjectId, }) => {
    // -- check if it's nodejs
    if ((0, misc_1.isNode)()) {
...
const _signAndGetAuth = async ({ web3, account, chainId, resources, expiration, uri, }) => {
 ...
 * @returns { AuthSig }
 */
const signAndSaveAuthMessage = async ({ web3, account, chainId, resources, expiration, uri, }) => {
    // check if it's nodejs
    if ((0, misc_1.isNode)()) {
